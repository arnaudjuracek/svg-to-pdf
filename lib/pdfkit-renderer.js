...
const resolveInerhitedProperties = require('./utils/svg-resolve-inerhited')
...
function render (doc, el, { colorSpace, userUnit }, drawCallback) {
  el = resolveInerhitedProperties(el)
...
module.exports = {
  inherit: (children, parent) => {
    ...
    children.forEach(child => {
      child.properties.inerhited = Object.assign({},
        /**
         * NOTE: Handle non-rendered → non-rendered elements inerhitance by
         * merging any pre-existing inerhited properties to its child, even if
  ...
  line: (doc, el, { colorSpace, userUnit }) => {
    render(doc, el, { colorSpace, userUnit }, el => {
