...
module.exports = {
  path: (doc, el, { colorSpace, userUnit }) => {
    ...
    // NOTE: add `d` path attribute coordinates are always unitless, we have to
    ...
    doc.path(el.properties.d).fillAndStroke()
  ...
  rect: (doc, el, { colorSpace, userUnit }) => {
    ...
    doc.rect(el.properties.x, el.properties.y, el.properties.width, el.properties.height).fillAndStroke()
